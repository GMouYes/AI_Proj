...
def ifValidZoneList(zoneList, map):
    # check if it exceed limit
    if countZone("I", zoneList) > map.maxIndustrial:
        return False
    if countZone("R", zoneList) > map.maxResidential:
        return False
    if countZone("C", zoneList) > map.maxCommercial:
        return False

    # Check if there are areas available for building
    siteList = map.siteList
    num_sites_built_on = len([site for site in siteList for zone in zoneList if site.location == zone.location])
    if len(zoneList) + len(siteList) - num_sites_built_on > map.mapState.shape[0] * map.mapState.shape[1]:
        return False
